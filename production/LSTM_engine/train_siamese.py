...
# ------------------------------
def semi_hard_negative_mining(pairs, labels, encoder, keep_ratio=0.3):

    print("🔍 Performing semi-hard negative mining...")

    emb_a = encoder.predict(pairs[:, 0], verbose=0)
    emb_b = encoder.predict(pairs[:, 1], verbose=0)
    ...
    neg_distances = distances[negative_idx]

    threshold = np.percentile(neg_distances, keep_ratio * 100)

    semi_hard = negative_idx[neg_distances <= threshold]

    selected_indices = np.concatenate([positive_idx, semi_hard])

    np.random.shuffle(selected_indices)

    print(f"   Kept {len(positive_idx)} positive pairs")
    print(f"   Kept {len(semi_hard)} semi-hard negative pairs")

    return pairs[selected_indices], labels[selected_indices]


# ------------------------------
# Cosine Gap Early Stopping
# ------------------------------
class SmartCosineStopping(Callback):

    def __init__(self, val_pairs, val_labels, encoder, patience=5):
        super().__init__()
        self.val_pairs = val_pairs
        self.val_labels = val_labels
        self.encoder = encoder
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        print(
            f"\n📊 Epoch {epoch+1} -> Gap: {gap:.4f} "
            f"(Match: {avg_match:.3f} | Imposter: {avg_imposter:.3f})"
        )
...
def main():
    ...
    print("🚀 Initiating Siamese Training Protocol\n")

    X_data, y_labels, label_map, real_windows, synthetic_windows, real_labels = load_all_users()

    if len(X_data) == 0:
        print("❌ CRITICAL: No data loaded. Exiting.")
        return

    real_users = [k for k in label_map.keys() if k.lower().startswith("person") and int(k.lower().replace("person","")) <= 10]
    synthetic_users = [k for k in label_map.keys() if k not in real_users]

    print("\n========== DATASET SUMMARY ==========")
    print(f"Real Users Loaded: {len(real_users)}")
    print(f"Synthetic Users Loaded: {len(synthetic_users)}")
    ...
    pairs, labels = generate_siamese_pairs(X_data, y_labels, real_labels)

    siamese_model, base_encoder = build_siamese_model(input_shape)

    pairs, labels = semi_hard_negative_mining(pairs, labels, base_encoder)
    ...
    print("🥞 Preparing tf.data pipelines...")
    ...
    print(f"\n🔥 Starting Training on {len(train_x)} pairs...\n")
